...
bool MongoDBAdapter::
init(const MongoDBAdapterConfig& config) {
...
query(const string& table_name, cursor_ptr& cursor, const query_type& query) {
	auto_ptr<DBClientCursor> c = conn_.query(ns_ + table_name, query.getData().data(),
